...
def lattice_generator_page():
    ...
    with (col1):
        ...
        Radius = st.number_input("Radius", value=0.1)
        ...
        with col1Grad:
            ...
            if GradDimRuleActivated:
                with st.container(border=True):
                    GradDimRule = st.selectbox("Rule", options=["linear", "parabolic", "sinusoide", "exponential"], index=0)
                    optionX = st.checkbox("X-axis", value=False)
                    if optionX:
                        GradDimParametersX = st.number_input("Parameters X", value=1.1)
                    else:
                        GradDimParametersX = 1.0
                    optionY = st.checkbox("Y-axis", value=False)
                    if optionY:
                        GradDimParametersY = st.number_input("Parameters Y", value=1.1)
                    else:
                        GradDimParametersY = 1.0
                    optionZ = st.checkbox("Z-axis", value=False)
                    if optionZ:
                        GradDimParametersZ = st.number_input("Parameters Z", value=1.1)
                    else:
                        GradDimParametersZ = 1.0
                    GradDimDirection = [int(optionX), int(optionY), int(optionZ)]
                    GradDimParameters = [GradDimParametersX, GradDimParametersY, GradDimParametersZ]
            else:
        ...
        with col2Grad:
            ...
            if GradRadRuleActivated:
                with st.container(border=True):
                    GradRadRule = st.selectbox("Rule", options=["linear", "parabolic", "sinusoide", "exponential"], index=0)
                    optionX = st.checkbox("X-axis", value=False)
                    if optionX:
                    ...
                    GradRadParameters = [GradRadParametersX, GradRadParametersY, GradRadParametersZ]
            else:
        ...
        cellBlocker = st.checkbox("Cell zone blocker", value=False)
        if cellBlocker:
            with st.container(border=True):
                col1Blocker, col2Blocker = st.columns(2)
                with col1Blocker:
                    st.write("Starting point:")
                    xPosStart = st.number_input("X coordinate start", value=0.0, step=1.0)
                    yPosStart = st.number_input("Y coordinate start", value=0.0, step=1.0)
                    zPosStart = st.number_input("Z coordinate start", value=0.0, step=1.0)
                with col2Blocker:
                    st.write("Ending point:")
                    xPosEnd = st.number_input("X lenght", value=0.0, step=1.0)
                    yPosEnd = st.number_input("Y lenght", value=0.0, step=1.0)
                    zPosEnd = st.number_input("Z lenght", value=0.0, step=1.0)
                erasedPart = [[xPosStart, yPosStart, zPosStart, xPosEnd, yPosEnd, zPosEnd]]
        else:
            erasedPart = []

        # uncertaintyNode = st.checkbox("Node uncertainty", value=0)
        uncertaintyNode = 0
    ...
    with col2:
        ...
        if generate or generateAutomatic:
            ...
            lattice = Lattice(cell_size_X, cell_size_Y, cell_size_Z, number_cell_X, number_cell_Y, number_cell_Z,
                              ...
                              hybridLatticeData=hybridLatticeData, hybridGeomType=[0, 16, 17], erasedParts=erasedPart)
